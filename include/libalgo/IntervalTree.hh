...
//
//You could easily transform it to an interval tree operating over range as big as sizeof(long long)
//
//Amount of memory and time consumed by the tree is pessimisticaly O(m*logn) where
//n is the size of the tree and m is the number of operations performed
//
//Interface: 
//
//node(int n) constructs a tree operating on range [1, n] (including 1 and n)
//node(int p, int q) constructs a tree operating on range [p, q] (including p and q)
...
class node{
	...
	private:
		...
		void maximalRangesFitting(int p, int q, vector<pair<node *, int> >& V, int value_inherited){
		...
		void update(){
			...
			if (rs != NULL)
				max_subtree_value = max(max_subtree_value, rs -> max_subtree_value + rs -> value);
			if (parent != NULL)
				parent -> update();
			}
	...
	public:
		...
		node(int l, int r, node * par) : value(0), max_subtree_value(0), l(l), r(r), ls(NULL), rs(NULL){
			parent = par;
			};
		...
		int query(int p, int q){
			...
			int result = -INFINITY;
			for(auto i : V)
				result = max(result, i.first -> max_subtree_value + i.first -> value + i.second);
			return result;
			}
